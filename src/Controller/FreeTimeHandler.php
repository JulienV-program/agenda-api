...
//    $endDay = new \DateTime('2019-08-09');
        ...
        foreach ($eventByDate as $index => $item) {
            ...
            } elseif (($index > 0 && $index < $len - 1) && $item->getStart()->getDateTime() > $eventByDate[$index - 1]->getEnd()->getDateTime()) {
                $free[] = [
                    "start" => $eventByDate[$index - 1]->getEnd()->getDateTime(),
                    "end" => $item->getStart()->getDateTime()
                ];
            } elseif ($index === ($len - 1) && $item->getEnd()->getDateTime() < $endDay) {
                if ($item->getStart()->getDateTime() > $eventByDate[$index - 1]->getEnd()->getDateTime()) {
